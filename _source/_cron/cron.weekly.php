<?php
	/* 	 _           ___ _     _   _____ _____ _____ 
		...
		File Description:
			Weekly cronjob to be executed by server.
	*/
	// Starting Procedure
	...
	// Starting Message
	echo "------ Weekly bugfishCMS Cronjob ------"._CRON_SPACE_;
	...
	if(is_dir($object["path"]."/_site/")) {
		...
		foreach(_HIVE_CRON_INIT_SCAN_ as $file) {
		    ...
		    if (is_dir($object["path"]."/_site/".$file."/_cron/_weekly") AND $file != "." AND $file != "..") {
				
				// Loop Through Cronjobs in Modules Cronjob Folders
			    foreach (glob($object["path"]."/_site/".$file."/_cron/_weekly/cron.*.php") as $filename){ 
			...
			foreach ($object["extensions_path"] as $filename) {
				...
				if (is_dir($filename."/_cron/_weekly")) {
					foreach (glob($filename."/_cron/_weekly/cron.*.php") as $filenamex){ 
	...
	$cron_log = new x_class_log($object["mysql"], _TABLE_LOG_CRON_, "weekly");
