<?php
	...
	*/ if(file_exists("../../settings.php")) { require_once("../../settings.php"); } else { @http_response_code(404); Header("Location: ../"); exit(); } 
		if(!_HIVE_ADMIN_SITE_) { hive__error("Deactivated", "Shadow Backend is deactivated in ruleset.php! Enable _HIVE_ADMIN_SITE_ in ruleset.php or in any module with this functionality to enable this files execution!", "", true, 401); } 
		if(_HIVE_ADMIN_SITE_ == _HIVE_MODE_ AND _HIVE_MODE_ == _HIVE_MODE_DEFAULT_) { hive__error("Impossible", "Currently you are not able to switch between front and backend, because the front end backend site module are the same.", "", true, 401); } 
?><!DOCTYPE html>
<html>
    <head>
        <title>Backend/Frontend Switch - CMS</title>
    ...
    <body>
        <article><div class="card">
            <h1>Backend/Frontend Switch</h1>
				<?php if(@$_SESSION[_HIVE_COOKIE_."hive_mode"] == _HIVE_ADMIN_SITE_) { $is_admin = true; } else { $is_admin = false; } ?>
				<p>You are currently <?php if(@$_SESSION[_HIVE_COOKIE_."hive_mode"] != _HIVE_ADMIN_SITE_) { echo "<b>NOT</b>"; } ?> in Backend Mode!</p>
				<?php if($is_admin) { 
					...
					?>
						<input type="submit" value="Switch to Frontend" name="adm_change_access" class="btn"> <a href="../../" class="btn">Back to Home</a><br />
					</form>
				<?php } else { 
					if(@$_GET["adm_change_access"]) {
					...
					?>
					<form method="get">
						<input type="submit" value="Switch to Backend" name="adm_change_access"  class="btn"> <a href="../../" class="btn">Back to Home</a><br />
					</form>			
				<?php } ?>
        </div></article>
    </body>
